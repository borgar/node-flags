...
//     silent: suppress error reporting
...
function build() {
  ...
  // trim
  svg.trim = function(s) { return s.trim(); }
  ...
  svg.Element.style = function(node) { 
    ...
    var css = ( node.childNodes.length > 0 ? node.childNodes[0].nodeValue : '' ) +
              ( node.childNodes.length > 1 ? node.childNodes[1].nodeValue : '' );
  ...
  // comment element, do nothing
  svg.Element.comment = function(node) {}
  svg.Element.comment.prototype = new svg.Element.ElementBase;

  svg.Element.metadata = function(node) {}
  svg.Element.metadata.prototype = new svg.Element.ElementBase;
  ...
  svg.loadXmlDoc = function(ctx, dom) {
    ...
    var draw = function() {
      ...
      if ( svg.opts['ignoreDimensions'] != true ) {
        ...
        if ( !e.style('width').hasValue() && !e.style('width').hasValue() ) {
          // no height or width set, attempt to get them from viewbox
          var viewBox = svg.ToNumberArray(e.attribute('viewBox').value);
          ctx.canvas.width  = Math.max( 0, viewBox[2] - viewBox[0] );
          ctx.canvas.height = Math.max( 0, viewBox[3] - viewBox[1] );
        }
      }
