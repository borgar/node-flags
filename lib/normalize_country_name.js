...
, 'germany_east': 'east_germany'
, 'germany_west': 'west_germany'
, 'micronesia_states': 'micronesia'
, 'yemen_north': 'north_yemen'
, 'yemen_south': 'south_yemen'
, 'faeroes': 'faroe_islands'
, 'johnston_island': 'johnston_atoll'
, 'st_christopher_and_nevis': 'st_kitts_and_nevis'
};
...
function normalize_country_name ( name ) {
  ...
  if ( /\bkampuchea\b/i.test( name ) ) {
    if ( /\bpeoples?\b/i.test( name ) ) {
      return 'kampuchea_pr';
    }
    //if ( /\b(democratic)\b/i.test( name ) ) {
    //  return 'kampuchea';
    //}
    return 'kampuchea';  // Democratic Kampuchea
  }
